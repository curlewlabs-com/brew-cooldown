...
candidate = BrewCooldown::Executor::Candidate.new(record).prepare(allow_hooks: true)
...
retained = candidate.runtime_dependencies.map do |dependency|
  ...
  BrewCooldown::Executor::Retained.new(Keg.new(path.realpath))
end
map = BrewCooldown::Executor::ExactMap.new([candidate, *retained])
map.activate
...
begin
  BrewCooldown::Executor.executing_name = candidate.formula.name
  begin
  ...
  raise "Guard probe changed inventory" unless inventory == BrewCooldown::Executor::Inventory.capture
ensure
  BrewCooldown::Executor.executing_name = nil
end
...
execution = BrewCooldown::Executor::Execution.new(map, state_directory: ENV.fetch("HOMEBREW_COOLDOWN_TEST_STATE"))
