...
require_relative "../../lib/brew_cooldown/executor/cask_retained"
...
if phase == "crash_check"
  ...
  raise "Inspection changed installed state" unless BrewCooldown::Executor::Inventory.capture == before
...
if phase == "accept"
  ...
  raise "Acceptance changed native packages" unless BrewCooldown::Executor::Inventory.capture == before
...
previous = BrewCooldown::Executor::CaskRetained.new("codex").cask
...
map.activate
BrewCooldown::Executor::CaskMap.new(candidate, predecessor: previous).activate
operation = BrewCooldown::Executor::CaskOperation.new(candidate, predecessor: previous)
execution = BrewCooldown::Executor::Execution.new(map, state_directory: state, casks: [operation])
before = BrewCooldown::Executor::Inventory.capture
...
when "drift"
  ...
  begin
    ...
    raise "Drift check changed installed state" unless BrewCooldown::Executor::Inventory.capture == changed
  ensure
    previous.unpin
  end
  raise "Pin cleanup changed native state" unless BrewCooldown::Executor::Inventory.capture == before
