...
require_relative "../../prototype/cask_map"
...
until pending.empty?
  ...
  dependency = BrewCooldown::Prototype::Retained.new(Keg.new(opt.realpath))
  retained[name] = dependency
  pending.concat(dependency.runtime_dependencies.map { |entry| entry.fetch("full_name") })
end
formula_map = BrewCooldown::Prototype::ExactMap.new(retained.values)
formula_map.activate
map = BrewCooldown::Prototype::CaskMap.new(candidate, predecessor:)
map.activate
...
raise "Guard check mutated inventory" unless BrewCooldown::Prototype::Inventory.capture == before
...
raise "Dependency check changed inventory" unless BrewCooldown::Prototype::Inventory.capture == before
...
formula_changes = BrewCooldown::Prototype::Inventory.differences(before, after).reject do |change|
