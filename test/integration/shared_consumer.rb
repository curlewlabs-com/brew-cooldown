...
candidates = all_records.fetch(phase).map do |record|
  BrewCooldown::Executor::Candidate.new(record).prepare(allow_hooks: true)
end
consumer = nil
receipt = nil
if phase == "baseline"
  ...
  consumer = BrewCooldown::Executor::Retained.new(Keg.new(HOMEBREW_CELLAR/"fish/4.7.1"))
...
map = BrewCooldown::Executor::ExactMap.new(candidates + [consumer].compact)
map.activate
if consumer
  begin
