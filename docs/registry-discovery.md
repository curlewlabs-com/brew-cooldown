...
to immutable index, platform manifest and bottle digests. A package that already
has the current build installed is never looked up here; see [command
planning](command-planning.md). It does not keep a release catalog. Mutable tag
lists and tag lookups are fetched afresh through Homebrew's curl helper;
immutable documents use Homebrew's resource cache and checksum verification.
